...
extension.setMessageListener(function(json) {
  ...
  if (typeof(callback) === 'function') {
    ...
    console.log('Invalid reply_id from Navigator system: ' + reply_id);
  }
});
...
exports.getSyncCPUInfo = function() {
  var cpuInfo = JSON.parse(_sendSyncMessage({
    'cmd': 'getSyncCPUInfo'
  }));
  if (cpuInfo['error']) {
    throw new tizen.WebAPIException(tizen.WebAPIException.NOT_SUPPORTED_ERR);
  } else {
    delete cpuInfo['error'];
    return _createConstClone(cpuInfo);
  }
};
