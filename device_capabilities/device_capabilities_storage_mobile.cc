...
//  FIXME (qjia7) Use event list to post message
//  const char* result_as_cstr = result.c_str();
//  AutoLock lock(&(instance->events_list_mutex_));
//  for (DeviceCapabilitiesEventsList::iterator it = storage_events_.begin();
...
void DeviceCapabilitiesStorage::OnStorageUnitAttach(keynode_t* node,
                                                    void* user_data) {
//  FIXME (qjia7) need to get the right valut to transfer to UpdateStorageUnits
